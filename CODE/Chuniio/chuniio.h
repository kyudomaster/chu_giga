...
CHUNIIO_EXPORT uint16_t chuni_io_get_api_version();
...
CHUNIIO_EXPORT HRESULT chuni_io_jvs_init();
...
CHUNIIO_EXPORT void chuni_io_jvs_poll(uint8_t *opbtn, uint8_t *beams);
...
CHUNIIO_EXPORT void chuni_io_jvs_read_coin_counter(uint16_t * out);
...
CHUNIIO_EXPORT HRESULT chuni_io_slider_init();
...
/* Start polling the slider. Your DLL must start a polling thread and call the
   ...
   Note that you do have to call the callback "occasionally" even if
   nothing is changing, otherwise the game will raise a comm timeout error.

   Minimum API version: 0x0100 */

CHUNIIO_EXPORT void chuni_io_slider_start(chuni_io_slider_callback_t callback);
...
CHUNIIO_EXPORT void chuni_io_slider_stop();
...
CHUNIIO_EXPORT void chuni_io_slider_set_leds(const uint8_t *rgb);
...
CHUNIIO_EXPORT HRESULT chuni_io_led_init();
...
CHUNIIO_EXPORT void chuni_io_led_set_colors(uint8_t board, uint8_t *rgb);
